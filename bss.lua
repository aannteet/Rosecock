...
local premiumtab = Window:CreateTab("Custom Features")
...
local buysection = premiumtab:CreateSection("Glider Speed")
guiElements["toggles"]["gliderspeed"] = buysection:CreateToggle("Enable", nil, function(State)
    kocmoc.toggles.gliderspeed = State
guiElements["vars"]["gliderspeed"] = gslider
end)
local gslider = buysection:CreateSlider("", 0, 400, 200, false, function(Value)
    kocmoc.vars.gliderspeed = Value
end)
...
game:GetService("RunService").Heartbeat:connect(function()
    if kocmoc.toggles.gliderspeed then
        local StatCache = require(game.ReplicatedStorage.ClientStatCache) local stats = StatCache:Get() stats.EquippedParachute = "Glider" local module = require(game:GetService("ReplicatedStorage").Parachutes) local st = module.GetStat local glidersTable = getupvalues(st) glidersTable[1]["Glider"].Speed = kocmoc.vars.gliderspeed setupvalue(st, st[1]'Glider', glidersTable)
    end
end)
